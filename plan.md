...
device nodes (membership of the `input` group — the usual desktop default; verified the
nodes are readable here).

### Design
- `src/input/mod.rs` — `GlobalInput` facade + the `device_query` backend.
...
- `input_probe` opened 9 devices and reported live EV_KEY + 1896 motion events — capture works end to end.
...
- A held key at startup or a device unplugged mid-press could leave a stuck key until next press;
  acceptable for a cosmetic overlay. Revisit if it bites.
