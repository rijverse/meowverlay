...
### Functional bugs
- [x] **1. Per-mode canvas size.** `skin.rs` now has `mode_bg_key()`; `canvas_size()` and
...
- [x] **6. Lock hotkey no longer fires mid-rebind.** `ui()` routes input to the binder *or* the
  hotkey, never both, while capturing; `handle_binding` ignores bare modifiers (Ctrl/Shift/Alt) so
...
- [x] **7. More keys bindable.** `keycode_to_vk` now covers every `device_query` 4.x `Keycode`
  (NumpadEnter/Decimal/Equals, F13–F20, Meta/Command/Option); `vk_to_label` has matching labels.
...
- [x] **8. License + asset attribution.** Added `LICENSE` (Apache-2.0) and `NOTICE` crediting
  bongocat-osu (MIT) for the default skin assets and config format; README has a License section.
...
- [x] **12. Stray working-tree change.** Already committed (`cursorSmoothing` is in
  `skins/default/config.json`); tree is clean.
