...
pub fn draw(frame: &Frame) {
    ...
    let left = if lr { "taiko_leftrim" } else if lc { "taiko_leftcentre" } else { "taiko_leftup" };
    if let Some(tex) = frame.skin.get(left) {
        canvas.draw_full(painter, tex);
    }

    let right = if rr { "taiko_rightrim" } else if rc { "taiko_rightcentre" } else { "taiko_rightup" };
